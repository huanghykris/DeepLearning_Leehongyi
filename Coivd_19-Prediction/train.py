...
def same_seed(seed):
    ...
    # 1. 设置CuDNN确定性模式（保证GPU计算确定性）
    torch.backends.cudnn.deterministic = True
    # 2. 关闭CuDNN自动优化（避免因算法选择导致结果差异）
    torch.backends.cudnn.benchmark = False
    # 3. 设置NumPy随机种子
    np.random.seed(seed)
    # 4. 设置CPU随机种子
    torch.manual_seed(seed)
    # 5. 如果存在GPU，设置所有GPU的随机种子
...
def select_feat(train_data, valid_data, test_data, select_all=True):
    # 从训练/验证数据中提取目标列（假设最后一列是y）
    y_train, y_valid = train_data[:, -1], valid_data[:, -1]

    # 提取原始特征列（训练/验证取除最后一列外所有列，测试集取全部）
    raw_x_train = train_data[:, :-1]  # 所有行，除最后一列
    raw_x_valid = valid_data[:, :-1]
    raw_x_test = test_data  # 测试集默认无目标列

    # 特征选择逻辑
    if select_all:
        feat_idx = list(range(raw_x_train.shape[1]))  # 选择所有特征（0到n-1列）
    else:
        feat_idx = [0, 1, 2, 3, 4]  # 手动选择特定特征（需根据实际数据调整）
...
def trainer(train_loader,valid_loader,model,config,device):
    ...
    for epoch in range(n_epochs):
        ...
        # tqdm is a package to visualize your train progress
